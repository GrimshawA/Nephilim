...
// Headers
////////////////////////////////////////////////////////////////////////////////
#include <SFMLAudioCode/SoundRecorder.h>
...
void SoundRecorder::start(unsigned int sampleRate)
{
    ...
    if (!isAvailable())
    {
        Log("Failed to start capture : your system cannot capture audio data (call SoundRecorder::IsAvailable to check it)");
        return;
    }
    ...
    if (captureDevice)
    {
        Log("Trying to start audio capture, but another capture is already running");
        return;
    }
    ...
    if (!captureDevice)
    {
        Log("Failed to open the audio capture device");
        return;
    }
...
void SoundRecorder::record()
{
    while (m_isCapturing)
    {
        ...
        sleep(Time::fromMiliSeconds(100));
    }
