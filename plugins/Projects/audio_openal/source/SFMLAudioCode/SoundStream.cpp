...
#include <SFMLAudioCode/SoundStream.h>
...
void SoundStream::setPlayingOffset(Time timeOffset)
{
    ...
    // Restart streaming
    m_samplesProcessed = static_cast<Uint64>(timeOffset.seconds() * m_sampleRate * m_channelCount);
    m_isStreaming = true;
    m_thread.launch();
}
...
Time SoundStream::getPlayingOffset() const
{
    if (m_sampleRate && m_channelCount)
    {
        ...
        return Time::fromSeconds(secs + static_cast<float>(m_samplesProcessed) / m_sampleRate / m_channelCount);
    }
    else
    {
        return Time::Zero;
