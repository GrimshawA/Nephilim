...
////////////////////////////////////////////////////////////////////////////////
bool SoundFile::openRead(const String& filename)
{
	...
	// -- Reading directly from the filesystem, proxy into a stream reader
	File* file = new File(filename, IODevice::BinaryRead);
	if(*file)
	{
		m_ownedStream = file;
		return openReadInternal(*file);
	}
	else
	{
		delete file;
   ...
   // DEPRECATED BECAUSE ALL FILES SHOULD BE OPEN FROM STREAMS
   ...
   /* if (m_file)
    ...
    if (!m_file)
    {
		Log("Failed to open sound file \"%s\" (%s)", filename.c_str(), sf_strerror(m_file));
        return false;
    }
...
/// Internal openRead from a stream for routing 
bool SoundFile::openReadInternal(File& stream)
{
	// If the file is already opened, first close it
	/*if (m_file)
		sf_close(m_file);*/
...
bool SoundFile::openRead(const void* data, std::size_t sizeInBytes)
{
    ...
    if (!m_file)
    {
        Log("Failed to open sound file from memory (%s)", sf_strerror(m_file));
        return false;
    }
...
void SoundFile::seek(Time timeOffset)
{
    if (m_file)
    {
        sf_count_t frameOffset = static_cast<sf_count_t>(timeOffset.seconds() * m_sampleRate);
