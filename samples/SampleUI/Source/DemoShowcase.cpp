...
RenderTexture userInterfaceTexture;
...
void SampleUI::onEvent(Event &event)
{
    ...
    if(event.type == Event::KeyPressed)
	{
		...
		{
			Image image = userInterfaceTexture.m_texture.copyToImage();
			if(image.saveToFile("ui.png"))
				Log("Saved UI to ui.png");
		}
	}
}
...
/*	getRenderer()->setModelMatrix(getCurrentTransform());
	...
	// Draw some ui
	getRenderer()->setTarget(userInterfaceTexture);
	getRenderer()->setDefaultTexture();
	...
	getRenderer()->setDefaultTarget();
	RectangleShape uiShape;
	uiShape.setSize(1024,768);
	uiShape.setColor(Color::White);
	uiShape.setTexture(&userInterfaceTexture.m_texture);
	uiShape.invertTextureCoordinates();
	getRenderer()->draw(uiShape);
}
