...
void Transform::setPosition(const vec2& p)
{
	position.x = p.x;
	position.y = p.y;
}
...
void Transform::setPosition(float px, float py)
{
	position.x = px;
	position.y = py;
}
...
void Transform::setLookAt(Vector3D target)
{
...
void Transform::rotateByEulerAngles(float x_axis, float y_axis, float z_axis)
{
	rotation.rotateEulerAngles(x_axis, y_axis, z_axis);
}

void Transform::rotateByAxisAngle(float angle, float x_axis, float y_axis, float z_axis)
{
